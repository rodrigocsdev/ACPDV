...
begin
 Result := Self;
 FConexao.SQL
   ('UPDATE ADQUIRENTE SET ID_POS=?, DESCRICAO=?, CNPJ=?, MERCHANTID=?' +
   ', CHAVE_REQUISICAO=? WHERE ID=?').Params(0, FAdquirente.GetIdPos)
   .Params(1, FAdquirente.GetDescricao).Params(2, FAdquirente.GetCnpj)
...
begin
 Result := Self;
 FDataSet := FConexao.SQL('INSERT INTO ADQUIRENTE ' +
   ...
   FAdquirente.GetChaveRequisicao)
   .ExecSQL.SQL
