...
begin
 Result := Self;
 FDataSet := FConexao.SQL('select * from Turno').Open.DataSet;
...
begin
 Result := Self;
 FDataSet := FConexao.SQL('select * from Turno where id=:id').Params('id', Id)
   .Open.DataSet;
end;
...
begin
 Result := Self;
 FConexao.SQL('Delete from Turno where id=:id').Params('id', aId).ExecSQL;
end;

function TDAOTurno.Excluir: iDAO<TTurno>;
begin
 Result := Self;
 FConexao.SQL('Delete from Turno where id=:id').ExecSQL;
end;
...
begin
 Result := Self;
 FConexao.SQL('UPDATE TURNO SET NOME=? WHERE ID= ?')
   .Params(0, FTurno.GetIdTurno).Params(1, FTurno.GetNome).ExecSQL;
end;

function TDAOTurno.Inserir: iDAO<TTurno>;
begin
 Result := Self;
 FDataSet := FConexao.SQL('INSERT INTO TURNO (NOME) VALUES(:NOME)')
   .Params(':NOME', FTurno.GetNome).ExecSQL.Open.DataSet;
end;
