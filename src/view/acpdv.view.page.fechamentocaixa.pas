...
uses
 ...
 acpdv.model.caixa,
 acpdv.model.enum,
 acpdv.controller, acpdv.view.componente.listatipopgfechamento;
...
type
 ...
 public
  ...
  function Informacoes(Value: TDictionary<String, Variant>)
...
function TPageFechamentoCaixa.Informacoes(Value: TDictionary<String, Variant>)
  : TPageFechamentoCaixa;
var
 lKey: String;
begin
...
begin
 ...
 FLista := TComponentListaFechamentoCaixa.New(Self)
   .TipoPagamento(ComboBox1.Text).Valor(Edit1.Text).Embed(ListBox1)
   .Nome('Frame' + FIndex.ToString).Click(RemoveIntemLista).Alinhamento(alTop);

 FFechamento.AddOrSetValue(ComboBox1.Text, StringReplace(Edit1.Text, 'R$', '',
   [rfReplaceAll]));
end;
...
function TPageFechamentoCaixa.ValidarCaixa(Value: TProc<Boolean>)
  : TPageFechamentoCaixa;
begin
 Result := Self;
 FProc := Value;
